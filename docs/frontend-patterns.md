...
```ts
beforeEach(() => {
  usePlayerStore.setState({ nowPlaying: null, isPlaying: false, volume: 50, progress: 0, partyMode: false });
...
```tsx
<Image src={track.albumArt} alt={track.album} width={48} height={48} unoptimized />
```
